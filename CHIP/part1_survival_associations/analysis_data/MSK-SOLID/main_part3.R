...
data_clinical_patient = fread(file="downloads/2024/msk_chord_2024/data_clinical_patient.txt",skip=4)
data_clinical_sample = fread(file="downloads/2024/msk_chord_2024/data_clinical_sample.txt",skip=4)
...
for(i in PATIENT_ID) {
    ...
    cont = cont + 1
    if(length(pos)==1) {
        AGE[cont] = as.numeric(data_clinical_patient$CURRENT_AGE_DEID[pos])
        GENDER[cont] = as.character(data_clinical_patient$GENDER[pos])
...
clinical_data = data.frame(PATIENT_ID=PATIENT_ID,SAMPLE_ID=SAMPLE_ID,AGE=AGE,GENDER=GENDER,RACE=RACE,SAMPLE_TYPE=SAMPLE_TYPE,METASTATIC_SITE=METASTATIC_SITE,CANCER_TYPE=CANCER_TYPE,CANCER_TYPE_DETAILED=CANCER_TYPE_DETAILED,OS_MONTHS=OS_MONTHS,OS_STATUS=OS_STATUS,check.names=FALSE,stringsAsFactors=FALSE)
clinical_data$GENDER[which(clinical_data$GENDER=="Unknown")] = NA
clinical_data$RACE[which(clinical_data$RACE%in%c("","No value entered","Pt refused to answer","Pt Refused To Answer","Unknown","No Value Entered"))] = NA
clinical_data$SAMPLE_TYPE[which(clinical_data$SAMPLE_TYPE=="Unknown")] = NA
...
data_mutations_extended = fread(file="downloads/2024/msk_chord_2024/data_mutations.txt")
...
data_cna_hg19 = fread(file="downloads/2024/msk_chord_2024/data_cna_hg19.seg")
...
data_cna = fread(file="downloads/2024/msk_chord_2024/data_cna.txt")
...
data_sv = fread(file="downloads/2024/msk_chord_2024/data_sv.txt")
...
# save the results
save(clinical_data,file="processed_data/2024/clinical_data.RData")
save(mutations_data,file="processed_data/2024/mutations_data.RData")
save(mutations,file="processed_data/2024/mutations.RData")
save(cna_data,file="processed_data/2024/cna_data.RData")
save(gistic,file="processed_data/2024/gistic.RData")
save(sv_data,file="processed_data/2024/sv_data.RData")
