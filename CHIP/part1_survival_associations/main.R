...
for(i in sort(unique(clinical_data$CANCER_TYPE))) {
    ...
    analysis_data = data.frame(cbind(survival_data[,c("OS_MONTHS","OS_STATUS","AGE","GENDER")],features_data))
...
names(model_estimates)[c(2,3)] = c("Breast Ductal Carcinoma","Breast Lobular Carcinoma")
names(model_estimates)[5] = "High-Grade Ovarian Cancer"
names(model_estimates)[15] = "Upper Urothelial Carcinoma"
